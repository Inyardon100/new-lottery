    ...
    with col2:
        ...
        if not st.session_state.admin_auth:
            ...
            if action == "새 추첨 생성":
                ...
                if df_m.empty: st.info("관리할 추첨이 없습니다.")
                else:
                    ...
                    st.markdown("---")
                    if st.button("삭제", key=f"delete_btn_{lid}"): st.session_state.delete_confirm_id = lid
                    if st.session_state.delete_confirm_id == lid:
                        ...
                        if st.button("예, 삭제합니다", key=f"confirm_del_btn_{lid}", type="primary"):
                            ...
                            st.session_state.delete_confirm_id = None; st.session_state.view_mode = 'list'; st.session_state.selected_lottery_id = None
