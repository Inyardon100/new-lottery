...
def run_draw(conn, lottery_id, num_to_draw, candidates):
    ...
    for winner in winners:
        c.execute("INSERT INTO winners (lottery_id, winner_name, draw_round) VALUES (?, ?, ?)", (lottery_id, winner, current_round))
...
# --- 3. Streamlit UI 구성 (롤백 및 최종 안정화 버전) ---
def main():
    ...
    with col1:
        ...
        if lotteries_df.empty:
            ...
            for index, row in lotteries_df.iterrows():
                ...
                title, status, draw_time = row['title'], row['status'], pd.to_datetime(row['draw_time'])
                with st.container(border=True):
                    ...
                    if not winners_df.empty:
                        ...
                        if st.session_state.get(f'celebrated_{lottery_id}', False):
                            st.balloons(); st.session_state[f'celebrated_{lottery_id}'] = False
                    else:
    ...
    with col2:
        ...
        # CSS를 이용한 UI 숨기기/보이기 방식 유지
        if st.session_state.admin_auth:
            login_style, admin_panel_style = "display: none;", "display: block;"
        else:
            login_style, admin_panel_style = "display: block;", "display: none;"
            ...
            if st.button("인증", key="auth_button"):
                try:
                    if password == st.secrets["admin"]["password"]:
                        ...
                        # st.rerun() 제거! Streamlit이 알아서 새로고침하게 둡니다.
                    else:
            ...
            if admin_action == "새 추첨 만들기":
                ...
                if st.button("✅ 추첨 생성", type="primary", key="create_button"):
                    ...
                    if not title or not participants:
                        ...
                        st.success("추첨이 생성되었습니다!"); time.sleep(1) # 메시지 확인 시간
                        # st.rerun() 제거!
                ...
                if 'lotteries_df' in locals() and not lotteries_df.empty:
                        ...
                        if not candidates:
                            ...
                            if st.button("🚀 재추첨 실행", type="primary", key="redraw_button"):
                                if final_candidates:
                                    ...
                                    st.success("재추첨이 완료되었습니다!"); time.sleep(1)
                                else:
                    ...
                    if st.session_state.delete_confirm_id == lottery_id:
                        st.warning(f"**경고**: '{choice}' 추첨의 모든 기록이 영구적으로 삭제됩니다. 정말로 삭제하시겠습니까?")
                        if st.button("예, 삭제합니다", type="primary", key=f"confirm_delete_btn_{lottery_id}"):
                            ...
                            conn.commit()
                            st.session_state.delete_confirm_id = None
                            st.success(f"'{choice}' 추첨이 완전히 삭제되었습니다."); time.sleep(2)
                else:
