...
def setup_database():
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS lotteries (
            id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, draw_time TIMESTAMP NOT NULL,
            num_winners INTEGER NOT NULL, status TEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)
    ''')
    c.execute('''
        CREATE TABLE IF NOT EXISTS participants (
            id INTEGER PRIMARY KEY AUTOINCREMENT, lottery_id INTEGER, name TEXT NOT NULL,
            FOREIGN KEY (lottery_id) REFERENCES lotteries (id) ON DELETE CASCADE)
    ''')
    c.execute('''
        CREATE TABLE IF NOT EXISTS winners (
            id INTEGER PRIMARY KEY AUTOINCREMENT, lottery_id INTEGER, winner_name TEXT NOT NULL, draw_round INTEGER NOT NULL,
            FOREIGN KEY (lottery_id) REFERENCES lotteries (id) ON DELETE CASCADE)
    ''')
    c.execute('''
        CREATE TABLE IF NOT EXISTS lottery_logs (
            id INTEGER PRIMARY KEY AUTOINCREMENT, lottery_id INTEGER, log_timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            log_message TEXT NOT NULL, FOREIGN KEY (lottery_id) REFERENCES lotteries (id) ON DELETE CASCADE)
    ''')
    # 데이터베이스 무결성을 위해 ON DELETE CASCADE 추가
    conn.commit()
    return conn

# 로그/추첨 로직 등은 변경 없음... (이하 생략)
# ...
...
def run_draw(conn, lottery_id, num_to_draw, candidates):
    ...
    if actual_num_winners <= 0: return []
...
# --- 3. Streamlit UI 구성 (관리자 패널 구조 변경) ---
def main():
    ...
    with col2:
        ...
        # ================================================================= #
        # 최종 해결책: 인증 상태에 따라 컨테이너를 통째로 보여주거나 숨김
        # 이래야 내부 위젯 렌더링 순서가 꼬이지 않음.
        ...
        if not st.session_state.admin_auth:
            ...
            if st.button("인증"):
                ...
                try:
                    ...
                    st.error("Secrets 설정을 확인해주세요. 관리자에게 문의하세요.")
                    st.error(f"오류 상세: {e}")
                ...
                if 'lotteries_df' in locals() and not lotteries_df.empty:
                    ...
                    if selected_lottery['status'] == 'completed':
                        ...
                        if not candidates:
                            ...
                            st.write("아래 명단에서 재추첨 대상을 선택하세요.")
                    ...
                    if st.button("🗑️ 추첨 삭제하기", type="secondary"):
                    ...
                    if st.session_state.get('delete_confirm_id') == lottery_id:
                        ...
                        if st.button("예, 삭제합니다", type="primary"):
                            ...
                            # 다른 테이블은 ON DELETE CASCADE로 자동 삭제됨
                            conn.commit()
